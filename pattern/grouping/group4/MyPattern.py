...
for thickness in range(40):
    ...
    top = np.hstack((matrix, inverted_matrix,matrix, inverted_matrix))
    bottom = np.hstack((inverted_matrix, matrix,inverted_matrix, matrix))
    
    top1 = np.hstack((matrix, inverted_matrix,matrix, inverted_matrix))
    bottom1 = np.hstack((inverted_matrix, matrix,inverted_matrix, matrix))
    final_matrix = np.vstack((top, bottom,top1,bottom1))
